...
clear;clear;clear
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "\033[96;1m              WELCOME TO SRICPT SCRIPTUN VIP              \033[0m"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 3
...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/script-VIP/izin/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/script-VIP/izin/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/script-VIP/izin/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "\e[32mloading...\e[0m"
clear
REPO="https://raw.githubusercontent.com/script-VIP/Vip/main/"
start=$(date +%s)
secs_to_human() {
...
echo ".:. VPN EXPRESS VIP .:." > /etc/xray/username
echo ""
}
clear
restart_system() {
...
wget -q raw.githubusercontent.com/script-VIP/Vip/main/Fls/limit.sh && chmod +x limit.sh && ./limit.sh
...
echo "SERVERPREMIUM" > /etc/handeling
...
wget -q -O /usr/bin/enc "https://raw.githubusercontent.com/script-VIP/Vip/main/Enc/encrypt" ; chmod +x /usr/bin/enc
...
wget https://raw.githubusercontent.com/script-VIP/Vip/main/Cfg/m-noobz 
